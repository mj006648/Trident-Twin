...
## Isaac Sim / Omniverse PoC

이 저장소에는 Trident Lakehouse의 `Lake → Metadata → Lakehouse → Staging → Workload Interface` 흐름을 NVIDIA Omniverse/Isaac Sim에서 확인할 수 있는 초기 PoC가 포함되어 있습니다.
...
| `stages/trident_lakehouse_twin_replay.usda` | mock event replay가 반영된 USD stage |
| `scripts/create_scene.py` | Isaac Sim Python 기반 stage 생성 스크립트 |
| `scripts/replay_events.py` | `data/mock_twin_events.json`을 USD time samples/custom attributes로 반영하는 replay 스크립트 |
| `data/twin_entities.json` | Lake, Lakehouse, metadata station, workload interface, dataset entity 정의 |
| `data/mock_twin_events.json` | 데이터셋 상태 전이 mock event |
...
Isaac Sim GUI에서는 아래 stage를 열어 확인합니다.
