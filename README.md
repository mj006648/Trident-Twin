...
> Public README에는 실제 운영 IP, host, user, container 이름, registry tag를 기록하지 않는다. 실제 값은 GitOps manifest, Secret, 또는 private runbook에서 관리한다.

| 항목 | 현재 값 |
| --- | --- |
| Portal image | `<portal-image>` |
| twin-hub image | `<twin-hub-image>` |
| GitOps source | `SmartX-Team/TwinX-Ops`의 ArgoCD manifest. 실제 클러스터 반영은 PR/merge로만 변경 |
| Portal service | `<portal-url>` |
| Portal → twin-hub | `<twin-hub-internal-url>` |
| twin-hub LoadBalancer | `<twin-hub-url>` |
| Portal → Isaac signaling | `<isaac-sim-host>:<signaling-port>` |
| Isaac container | `<user>@<isaac-host>`의 Docker container `<isaac-container>` |
| Scene artifact | `<trident-twin-workdir>/stages/trident_lakehouse_twin_*.usda` 중 운영자가 선택한 최신 생성본 |

운영 확인 항목:

- `python3 -m py_compile` for `scripts/*.py`, `twin-hub/app.py`, Isaac extension
- `python3 scripts/draw_site_plan.py`
- Isaac capture: latest USD 기준 actual scene screenshots 갱신
- Isaac stream restart: `[ext: trident.twin-0.1.0] startup`, `Isaac Sim Full Streaming App is loaded`
- Cluster rollout: `<portal-image>`, `<twin-hub-image>`
...
### 2.1 Scene site plan / 설계도

![Trident Twin site plan](docs/site-plan.png)

### 2.2 실제 Isaac scene screenshots
...
```bash
# <user>@<isaac-host> SSH 이후, 또는 docker exec로 실행
cd "<trident-twin-workdir>"
/isaac-sim/python.sh scripts/capture_overview.py
```

> 주의: actual capture는 Isaac renderer를 사용하므로 보통 Digital Twin stream process를 잠깐 멈춘 뒤 실행하고, 캡처 후 stream을 다시 켠다. 다른 Isaac container/process는 건드리지 않는다.
...
Portal /digital-twin
  ...
  │    └─ ISAAC_SIM_HOST=<isaac-sim-host>, ISAAC_SIM_SIGNALING_PORT="<signaling-port>"
  ├─ IsaacSimViewer
...
| --- | --- | --- | --- |
| WebRTC stream | Portal Digital Twin 탭이 열려 있을 때 Isaac stream 유지 | 해당 없음 | stale stream이면 Digital Twin stream process만 재시작 |
...
| 변수 | 예시 |
| --- | --- |
| `GEMMA4_BASE_URL` | `<gemma4-base-url>` |
| `GEMMA4_MODEL` | `gemma4` |
...
1. `TWIN_HUB_URL` 또는 `TRIDENT_TWIN_HUB_URL`을 읽는다.
   - 기본값은 fixture mode이며, live 연동 시 `<twin-hub-url>`을 명시한다.
...
```text
stages/trident_lakehouse_twin_*.usda
```
...
```bash
SCENE="<trident-twin-workdir>/stages/trident_lakehouse_twin_*.usda"
...
## 13. Isaac Sim runbook

현재 Isaac Sim은 별도 GPU host의 Docker container에서 실행한다. 실제 host, user, container 이름은 public README에 기록하지 않는다.

중요 원칙:

- 다른 Isaac container/process는 건드리지 않는다.
- Digital Twin용 container인 **`<isaac-container>`만** 확인/재시작한다.
- 가능하면 Docker container 자체 재시작보다 container 내부 `runheadless.sh`/`kit` stream process만 재시작한다.

상태 확인:

```bash
ssh "<user>@<isaac-host>" 'bash -s' <<'REMOTE'
CONTAINER="<isaac-container>"
SIGNALING_HOST="<isaac-sim-host>"
SIGNALING_PORT="<signaling-port>"
MEDIA_PORT="<media-port>"

docker ps --filter name="$CONTAINER" --format 'table {{.Names}}	{{.Status}}	{{.Ports}}'
docker exec "$CONTAINER" bash -lc \
  'ps -eo pid,cmd | grep -E "[/]isaac-sim/kit/kit|[r]unheadless.sh" || true'
ss -ltnup | grep -E "${SIGNALING_HOST}:(${SIGNALING_PORT}|${MEDIA_PORT})" || true
REMOTE
```

Digital Twin 화면이 `Isaac Sim 스트리밍 서버에 연결 중… <isaac-sim-host>:<signaling-port>`에서 오래 멈추면, 포트는 열려 있지만 WebRTC stream process가 stale 상태일 수 있다. 이때는 Digital Twin container 안의 stream process만 재시작한다.

```bash
ssh "<user>@<isaac-host>" 'bash -s' <<'REMOTE'
set -euo pipefail
CONTAINER="<isaac-container>"
WORK="<trident-twin-workdir>"
TWIN_HUB_URL="<twin-hub-url>"
PUBLIC_ENDPOINT="<isaac-sim-host>"
CONTAINER_USER="<container-user-id>"
LOG=/tmp/trident-streaming-restart.log

# Stop only the Digital Twin stream process, not other containers.
PIDS=$(docker exec "$CONTAINER" bash -lc "pgrep -f '/isaac-sim/kit/kit|runheadless.sh' || true")
if [ -n "${PIDS:-}" ]; then
...
docker exec -d -u "$CONTAINER_USER" "$CONTAINER" bash -lc \
  "cd /isaac-sim && TWIN_HUB_URL=$TWIN_HUB_URL ./runheadless.sh \
    --/app/livestream/publicEndpointAddress=$PUBLIC_ENDPOINT \
    --ext-folder $WORK/exts \
    --enable trident.twin > $LOG 2>&1"

for i in $(seq 1 180); do
  if docker exec -u "$CONTAINER_USER" "$CONTAINER" bash -lc "grep -q 'Full Streaming App is loaded' $LOG"; then
...
```bash
TRIDENT_TWIN_HUB_IMAGE="<registry>/trident-twin-hub:<tag>"
docker build -f Dockerfile.twin-hub -t "$TRIDENT_TWIN_HUB_IMAGE" .
docker push "$TRIDENT_TWIN_HUB_IMAGE"
```
...
```bash
npm run build
TRIDENT_PORTAL_IMAGE="<registry>/trident-portal:<tag>"
docker build -t "$TRIDENT_PORTAL_IMAGE" .
docker push "$TRIDENT_PORTAL_IMAGE"
```
