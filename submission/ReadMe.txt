...
- 실제 연동 데모
  - AI대학원/SmartX 내부망에서 Trident Portal에 접속해 발표 때와 같은 흐름을 확인합니다.
  - Kubernetes, ArgoCD, Keycloak, Lakehouse backend, Gemma4, Isaac Sim WebRTC stream은 운영 환경에 이미 연결되어 있습니다.
- 외부 PC 실행
  - 클러스터 없이 제출 패키지 안의 USD/USDA scene, Isaac Sim extension, twin-hub fixture 데이터로 기본 동작을 확인합니다.

전체 live mode를 외부 PC에 새로 구성하려면 설치해야 하는 컴포넌트가 많습니다. 그래서 실제 연동 검증은 내부망 Portal에서 진행하고, 외부 PC에서는 scene과 extension 중심으로 확인할 수 있게 구성했습니다.

2. 제출 패키지 구성
- ReadMe.txt
  - 이 파일입니다. 실행 방법, 버전 정보, 포함 파일, 실제 데모 재현 절차를 설명합니다.
...
- integration/
  ...
  - 외부 컴퓨터 단독 실행에는 필수는 아니며, 실제 클러스터 배포와 연동 구조를 확인할 때 참고합니다.
...
4. 실제 연동 데모 재현 방법
발표 때 시연한 흐름은 AI대학원/SmartX 내부망의 Portal에서 확인할 수 있습니다.

4.1 접속 조건
- AI대학원/SmartX 내부망 또는 내부망에 접속 가능한 환경이 필요합니다.
...
4.3 실제 연동 구성
Portal live mode는 아래 구성요소가 배포된 환경에서 동작합니다.
...
외부 컴퓨터에서 live mode 전체를 새로 구성하려면 integration 폴더의 Portal API 파일과 ArgoCD manifest를 참고하면 됩니다. 다만 이 경우는 단순 실행이 아니라 운영 환경을 다시 배포하는 작업에 가깝습니다.

5. 외부 컴퓨터에서 fixture mode 최소 실행 방법
...
7) fixture mode에서는 실제 Portal/Gemma4/Lakehouse 없이 mock entity와 command bridge 구조를 확인할 수 있습니다.
